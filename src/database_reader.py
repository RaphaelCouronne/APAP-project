...
def customer_query(**kwargs):
    ...
    binary_kwargs = {key : val for key,val in kwargs.items() if key in ["software_skills"]}
    feature_kwargs = {key : val for key,val in kwargs.items() if key not in ["software_skills"]}
